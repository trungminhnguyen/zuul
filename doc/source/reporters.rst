...
reporter. It has the following options.
  ...
  **commit_status**
  ...
  set commit status on github. Defaults to ``true``.
  ``commit_status=true``

  **comment**
  Boolean value (``true`` or ``false``) that determines if the reporter should
  add a comment with the pipeline status to the github pull request.
  It defaults to ``true`` for **success**, **failure** and **merge-failure**
  reporters. It defaults to ``false`` for **start** reporter.
  ``comment=true``
